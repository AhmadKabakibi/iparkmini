...
const calculatePrice = function (parkingtime, date) {
  ...
  if (hoursDiff > 0) {
    let totalAmount = hoursDiff * units.PRICE
    let discount = hoursDiff > units.DISCOUNT_Duration ? (hoursDiff - units.DISCOUNT_Duration) * units.DISCOUNT : 0
    return {
    ...
    cache.incrAsync(parkinglotKey)
      .then((total) => {
        ...
        if (Number(total) > units.MAX_CARS) {
          ...
          const parking = new Parking(params)
          parking.save((error) => {
            if (error) {
